...
import { findAllScreenTime, findScreenTimeById, newScreenTime, updateScreenTime} from "../repositories/screenTimeRepository.js";
...
export const addScreenTime = async (data, userId) => {
    ...
    const isExist = allScreenTime.find(st => st.createdAt.toLocaleDateString() === new Date().toLocaleDateString());
    if(isExist) throw new Error(`Screen time already exist at date ${new Date().toLocaleDateString()}`);
...
export const updateScreenTimeById = async (updateData, screenTimeId, userId) => {
    const isExist = await findScreenTimeById(screenTimeId, userId);
    if(!isExist) throw new Error(`Screen time with id ${screenTimeId} at user id ${userId} not found!`);

    const inputData = await updateScreenTime(updateData, screenTimeId, userId);
    const result = {date: inputData.createdAt.toLocaleDateString(), ...inputData};
    return result;
}